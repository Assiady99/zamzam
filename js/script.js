...
window.addEventListener('scroll', debounce(applyParallax, 5));
...
window.addEventListener('load', () => {
    ...
    setTimeout(() => {
        setVh();
        initializeCriticalElements();
        applyParallax(); // Apply parallax after final viewport adjustment
    }, 200);
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Apply parallax effect immediately after initialization
    applyParallax();

    // Force layout recalculation to fix positioning issues
    setTimeout(() => {
        window.dispatchEvent(new Event('resize'));
        applyParallax(); // Reapply parallax after resize event
        forceLayoutFix(); // Reapply layout fix
    }, 100);
